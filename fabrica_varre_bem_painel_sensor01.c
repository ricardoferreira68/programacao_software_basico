...
// Vetor com os valores (rótulos) do sensor 01 para ser plotado sobre os pontos no gráfico.
...
int main(int argc, char** argv)
{
    ...
    else
    {
        ...
        glutInitWindowPosition(700,10);  // Define a posição na tela de exibição da janela gráfica.
    ...
    for (int i=0; i<CORTES; i++) 
    {
        if (i==0)
            ...
            if (i==1)
                ...
                Vx[i] = Vx[i-1]+125;  //Valor da ordenada 'x' do próximo corte a ser plotado.
    }
...
void desenha_grafico(void)
{
    ...
    desenha_texto("22",    x, y, 0.502, 0.502, 0.502);  // Desenha o texto "22" (cor cinza).
    y = My[0][1]; // O rótulo do eixo 'y' será plotado na posição 'My' da linha "Medida" ideal.
    desenha_texto("20",    x, y, 0.502, 0.502, 0.502);  // Desenha o texto "20" (cor cinza).
    y = My[0][2]; // O rótulo do eixo 'y' será plotado na posição 'My' da linha "Mínima".
    desenha_texto("19",    x, y, 0.502, 0.502, 0.502);  // Desenha o texto "19" (cor cinza).
    y = My[0][2]-100; // O rótulo do eixo 'y' será plotado na posição do eixo base do gráfico.
    desenha_texto("Corte", x, y, 0.502, 0.502, 0.502);  // Desenha o texto "Corte" (cor cinza).
    ...
    desenha_texto("Ideal",   x+25, y, 0, 0, 1);  // Desenha o texto "Ideal" no gráfico.
    ...
    desenha_texto("Minima",   x+25, y, 1, 0, 0);  // Desenha o texto "Mínima" no gráfico.

    x += legenda_tam; // Deslocamento no eixo 'x' para a plotagem do próximo item da legenda.
    desenha_linha(x, y+4,  x+25, y+4, 1, 1, 0, 4);  // Desenha a linha da legenda.
    desenha_texto("Maxima",   x+25, y, 1, 1, 0);  // Desenha o texto "Máxima" no gráfico.

    x += legenda_tam; // Deslocamento no eixo 'x' para a plotagem do próximo item da legenda.
    desenha_linha(x, y+4,  x+25, y+4, 0, 1, 0, 4);  // Desenha a linha da legenda.
    desenha_texto("Sensor01", x+25, y, 0, 1, 0);  // Desenha o texto "Sensor01" no gráfico.

    // Plotagem do número do corte no eixo 'x'.
    sprintf(str_valor_ponto, "%03d", contador_de_registro);  // Converte o valor para string.
    desenha_texto(str_valor_ponto, Vx[corte], 35, 0.502, 0.502, 0.502);  // plota o número do corte (cor cinza).
}
...
void desenha_texto(char *string, GLfloat xf, GLfloat yf, GLfloat r, GLfloat g, GLfloat b)
{
    ...
    while(*string)
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *string++);  // Renderiza o caractere.
