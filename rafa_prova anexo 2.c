...
// Um 'corte' significa um registro de dados lidos dos sensores e projetado nográfico. 
#define CORTES 6 // O 'corte 0' é usado para marcar o ponto inicial daslinhas. 
...
typedef struct 
{
 ...
 int s03; // Medida do sensor 03 (presença da fibra) 1-presente/2-ausente.
...
GLfloat y_ini = 300; // Posição da primeira linha (superior) a ser plotada noeixo 'y'.
// Vetores com as coordenadas de platagem das linhas do gráfico.
GLfloat Vx[CORTES]; // As ordenadas x de cada "Corte" a ser protado nográfico.
GLfloat My[CORTES][4]; // As ordenadas y de cada "Corte" e cada uma das 4linhas a serem plotados no gráfico.
// Vetor com os valores (rótulos) do sensor 01 para ser plotado sobre os pontosno gráfico.
...
int main(int argc, char** argv)
{
 ...
 glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB); // Define o módo de exibiçãodas imagens geradas.
 glutInitWindowSize(670,360); // Define o tamanho da janela gráficainicial.
 glutInitWindowPosition(700,10); // Define a posição na tela de exibição dajanela gráfica.
 glutCreateWindow("Monitor Sensor 01 da Guilhotina"); // Cria a janela comas configurações iniciais.
 inicializa(); // Define os valores iniciais.
 glutDisplayFunc(desenha_grafico); // Desenha os gráficos na janela criada.
 glutTimerFunc(TEMPORIZADOR, temporizacao, 1); // Força a exibição dográfico de 5 em 5 segundos.
 glutMainLoop(); // Mantém a janela em exibição (execução).
 return 0;
}
// Usada para fazer as configuração dos valores iniciais de controle dográfico.
void inicializa(void)
{
 // Inicia o controle de contagemm de registros lidos do arquivo.
 contador_de_registro = 0; // Nenhum registro lido.
 
 // Define as posições das ordenadas 'x' dos cortes a serem plotados nográfico.
...
corte a ser plotado.
 ...
 Vx[i] = Vx[i-1]+120; //Valor da ordenada 'x' do próximo corte
a ser plotado.
 }
 // Configurações Iniciais da janela gráfica.
 glOrtho(0, 670, 0, 360, -1 ,1); // Define o sistema de coordenadas da área
gráfica de exibição. glClearColor(1.0, 1.0, 1.0, 1.0); // Cor de fundo da janela (branca).
...
void desenha_grafico(void)
{
 GLfloat x, y; // Auxiliar nas definições de posição dos elementos do
gráfico.
 int legenda_tam; // espaço usado por cada item na legenda.
 char str_valor_ponto[CORTES]; // Armazena o valor do ponto do sensor01
como string.
 // Lê o próximo registro de dados do arquivo controlado por
'contador_de_registro'.
 ...
 // Localiza um registro no arquivo a partir da posição SEEK_SET (início
do arquivo).
 fseek(arq, contador_de_registro*sizeof(dados), SEEK_SET); 
 // Registro localizado.
 // Lê o registro de dados do sensor 01 referento ao corte da
guilhotina.
 fread(&dados, sizeof(dados), 1, arq);
 // Verifica se foi lido um registro válido.
 if (!feof(arq))
 {
 // Incrementa o contador de registro para apontar para o próximo
registro.
 contador_de_registro++;
 // Verifica se já foram preenchidas tadas as posições de CORTES no
gráfico.
 if (corte==CORTES-1)
 {
 // Faz a transferência dos valores para liberar a última
posição para a próxima leitura.
 for (int i=0;i<corte;i++)
 {
 rotulo_s01[i] = rotulo_s01[i+1]; // Transfere o valor do
rótulo.
 My[i][3] = My[i+1][3]; // Transfere os valores do 'corte'
para liberar a última posição.
 }
 My[0][3] = My[1][3]; // Repete o conteúdo da posição 1 na
posição 0. 
 }
 else
...
linha valor "Sensor01".
 ...
 x = x_ini - 60; // O rótulo do eixo 'y' será plotado na posição inicial
'x_ini' menos 60 pixels. 
 y = My[0][0]; // O rótulo do eixo 'y' será plotado na posição 'My' da linha
"Máxima".
 desenha_texto("22", x, y, 0.502, 0.502, 0.502); // Desenha o texto "22"
(cor cinza).
 y = My[0][1]; // O rótulo do eixo 'y' será plotado na posição 'My' da linha
"Medida" ideal.
 desenha_texto("20", x, y, 0.502, 0.502, 0.502); // Desenha o texto "20"
(cor cinza).
 y = My[0][2]; // O rótulo do eixo 'y' será plotado na posição 'My' da linha
"Mínima".
 desenha_texto("19", x, y, 0.502, 0.502, 0.502); // Desenha o texto "19"
(cor cinza).
 y = My[0][2]-100; // O rótulo do eixo 'y' será plotado na posição do eixo
base do gráfico.
...
do eixo x (cinza).
 ...
 // Desenha a legenda.
 x = 90; y = 20; // Posição inicial do primeiro item da legenda. legenda_tam = 120; // Define o tamanho de cada item na legenda para 
...
gráfico.
 x += legenda_tam; // Deslocamento no eixo 'x' para a plotagem do próximo
item da legenda.
 desenha_linha(x, y+4, x+25, y+4, 1, 0, 0, 4); // Desenha a linha da
legenda.
 desenha_texto("Minima", x+25, y, 1, 0, 0); // Desenha o texto "Mínima"
no gráfico.
 x += legenda_tam; // Deslocamento no eixo 'x' para a plotagem do próximo
item da legenda.
 desenha_linha(x, y+4, x+25, y+4, 1, 1, 0, 4); // Desenha a linha da
legenda.
 desenha_texto("Maxima", x+25, y, 1, 1, 0); // Desenha o texto "Máxima"
no gráfico.
 x += legenda_tam; // Deslocamento no eixo 'x' para a plotagem do próximo
item da legenda.
...
no gráfico.
 // Plotagem do número do corte no eixo 'x'.
 sprintf(str_valor_ponto, "%03d", contador_de_registro); // Converte o
valor para string.
 desenha_texto(str_valor_ponto, Vx[corte], 35, 0.502, 0.502, 0.502); //
plota o número do corte (cor cinza).
}
// Usada para desenha a borda cinza do gráfico.
void denha_borda_cinza(void)
{
 glClear(GL_COLOR_BUFFER_BIT); // Limpa a janela de exibição com a cor de
fundo especificada.
 glLineWidth(4); //Define a espessura da linha em 4 pixels.
 glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); // Define as cores de trás e e
da linha do polígono.
...
(cinza).
 ...
 glVertex2f(660, 350); // Linha vertical do lado direito até a altura
355.
 glVertex2f(10, 350); // Linha horizontal até x=5 (fecha
